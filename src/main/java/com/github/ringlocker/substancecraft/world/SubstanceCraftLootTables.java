...
public class SubstanceCraftLootTables {
    ...
    private static void addCornSeedsToVillagerHouseChestLoot() {
        LootTableEvents.MODIFY.register((lootTable, tableBuilder, lootTableSource, provider) -> {
            List<ResourceKey<LootTable>> chests = List.of(
                    BuiltInLootTables.VILLAGE_PLAINS_HOUSE, BuiltInLootTables.VILLAGE_SAVANNA_HOUSE, BuiltInLootTables.VILLAGE_DESERT_HOUSE, BuiltInLootTables.VILLAGE_SNOWY_HOUSE
            );
            if (lootTableSource.isBuiltin() && chests.contains(lootTable)) {
                ...
                lootPool.setRolls(ConstantValue.exactly(1.0F))
                        .add(LootItem.lootTableItem(SubstanceCraftBlocks.getBlockItem(SubstanceCraftBlocks.CORN_CROP)).setWeight(7))
    ...
    private static void addFungiToCrops() {
        ...
        LootTableEvents.MODIFY.register((lootTable, tableBuilder, lootTableSource, provider) -> {
            if (POTATO_LOOT_TABLE.isPresent() && lootTableSource.isBuiltin() && POTATO_LOOT_TABLE.get().equals(lootTable)) {
                LootItemCondition.Builder fullyGrownCondition = LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.POTATOES)
                ...
                LootItemCondition.Builder fullyGrownCondition = LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.CARROTS)
                        .setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(PotatoBlock.AGE, 7));
                LootPool.Builder lootPool = new LootPool.Builder();
                lootPool.when(fullyGrownCondition)
                        .add(LootItem.lootTableItem(SubstanceCraftItems.ERGOT).when(LootItemRandomChanceCondition.randomChance(0.0015F)))
                ...
                LootItemCondition.Builder fullyGrownCondition = LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.WHEAT)
                        .setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(PotatoBlock.AGE, 7));
                LootPool.Builder lootPool = new LootPool.Builder();
                lootPool.when(fullyGrownCondition)
                        .add(LootItem.lootTableItem(SubstanceCraftItems.ERGOT).when(LootItemRandomChanceCondition.randomChance(0.0015F)))
                ...
                LootItemCondition.Builder fullyGrownCondition = LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.BEETROOTS)
                        .setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(PotatoBlock.AGE, 7));
                LootPool.Builder lootPool = new LootPool.Builder();
                lootPool.when(fullyGrownCondition)
                        .add(LootItem.lootTableItem(SubstanceCraftItems.ERGOT).when(LootItemRandomChanceCondition.randomChance(0.0015F)))
